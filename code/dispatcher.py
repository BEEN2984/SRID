...
class Dispatcher():
    ...
    def Clean(self, ttl_s: int = 3600):
        # 3600s 마다 실행을 메인에 넣기
        now_ts = datetime.now().timestamp()

        # IP 정리
        for ip, ipstat in list(self.IP_table.items()):
            if ipstat.lastseen and (now_ts - ipstat.lastseen) > ttl_s:
                del self.IP_table[ip]

        # User 정리
        for user, userstat in list(self.User_table.items()):
            if userstat.lastseen and (now_ts - userstat.lastseen) > ttl_s:
...
class IPStats:
    ...
    def _add_event(self, event:parser.RawLogEvent, window_s:int=DEFAULT_WINDOW_SECONDS):
        ...
        # Evict old events
        while self.event_history and (self.lastseen-self.event_history[0][0] > window_s):
...
class UserStats:
    ...
    def _add_event(self, event:parser.RawLogEvent, window_s:int=DEFAULT_WINDOW_SECONDS):
        ...
        self.firstseen = self.event_history[0][0]
        self.unique_ip_count = len({ip for _, ip in self.event_history})

    def __str__(self):
        return (
            f"[UserStats] user={self.user} "
            f"fail={self.fail_count} "
            f"unique_ip_count={self.unique_ip_count}"
            f"firstseen={self.firstseen} "
            f"lastseen={self.lastseen}"
        )